...
router.get("/completion", function (req, res) {
  ...
  jfn = req.query.jf;
  if (jfn.match(/forge/gim) != null) {
    version = jfn.match(/1\.\d+\.\d+/gim);
    if (version != null) {
      version = version[0];
  ...
  if (process.platform == "win32") {
    if (newForgeInstalling == true) {
      run_read = fs
        .readFileSync("./servers/" + req.query.server + "/run.bat")
        .toString();
      ...
      if (jl != null) {
        ...
        fs.writeFileSync(
          ...
          "@echo off\nchcp 65001>nul\ncd servers\ncd " +
            req.query.server +
            "\n" +
            Buffer.from(req.query.startcmd, "base64")
              .toString()
              .replace("-jar", "")
              .trim() +
            " @" +
    ...
    if (newForgeInstalling == true) {
      run_read = fs
        .readFileSync("./servers/" + req.query.server + "/run.sh")
        .toString();
      run_read = run_read.split("\n");
      jl = null;
      run_read.forEach(function (line) {
...
router.get("/saveStartScript", (req, res) => {
  ...
  if (perms.includes(ACCESS_PERMISSION)) {
    if (typeof serverjson_cfg[req.query.server] !== "undefined") {
      res.send(
        serverController.saveStartScript(
          req.query.server,
          req.query.script,
          req.query.resonerr
        )
...
router.get("/saveServerPropertiesFile", function (req, res) {
  ...
  if (perms.includes(ACCESS_PERMISSION)) {
    if (typeof serverjson_cfg[req.query.server] !== "undefined") {
      res.send(
        serverController.saveServerProperties(req.query.server, req.query.doc)
      );
    } else {
      res.send("false");
    }
...
router.get("/start", function (req, res) {
  if (
    typeof serverjson_cfg[req.query.server] !== "undefined" &&
    serverjson_cfg[req.query.server].status == "stopped"
  ) {
...
router.get("/restart", function (req, res) {
  if (
    typeof serverjson_cfg[req.query.server] !== "undefined" &&
    serverjson_cfg[req.query.server].status == "started" &&
    typeof restart_after_stop[req.query.server] == "undefined"
  ) {
    restart_after_stop[req.query.server] = true;
    command = "stop";
    command = Buffer.from(command, "utf-8").toString();
    servers_logs[req.query.server] =
      servers_logs[req.query.server] + command + "\n";
