    ...
    if (result.isConfirmed) {
      ...
      $.ajax({
        url:
          ...
          window.localStorage.selectedServer,
        error: function (err) {
          $("#restoreBackupModal .modal-body").text(
            "Error happend:" + err.toString()
          );
        },
        success: function () {
    ...
    if (typeof data == "object") {
      data.forEach(function (item, i) {
        ...
        if (
          ...
          item.processing_status.status == "completed"
        ) {
          name_badge = "";
          name_size =
            "<span class='ms-2 gray-addon' style='font-family: monospace;'>" +
            size +
            "</span>";
          rr = "";
          clr = " color: white;";
          ...
          if (item.processing_status.status == "processing") {
            name_badge =
              '<span class="badge rounded-pill badge-primary text-primary ms-2">' +
              bks_st_proc +
              " (" +
            ...
            clr =
              " background: -webkit-linear-gradient(var(--mdb-gray-400) " +
              prcc +
              "%, var(--mdb-primary) " +
              prcc +
            ...
            name_badge =
              '<span class="badge rounded-pill badge-primary text-danger ms-2">' +
              bks_st_error +
              "</span>";
            name_size = "";
            rr =
              '<span style="margin: 0 8px; color: rgb(200,200,200)">|</span><span>{{backups-text-status}}: ' +
              bks_st_error +
              " " +
            ...
            rr =
              '<span style="margin: 0 8px; color: rgb(200,200,200)">|</span><span>{{backups-text-status}}: ' +
              bks_st_start +
              '</span><span style="margin: 0 8px; color: rgb(200,200,200)">|</span>';
            clr = " color: white;";
            icn = "cloud_done";
          }
        }
        ...
        $("#backups-list").append(
          '<li class="list-group-item px-3 w-100 border-0 rounded-3 list-group-item-light mb-2 d-flex flex-row align-items-center text-white"><span class="material-symbols-outlined" style="font-size: 48px; margin-right: 24px;' +
            clr +
            '">' +
            icn +
            '</span><div class="d-flex flex-column justify-content-center flex-fill"><span style="font-size: 17pt; font-weight: 600;">' +
            item.name +
            name_badge +
            name_size +
            '</span><div class="d-flex flex-row align-items-center gray-addon"><span>{{selected-sn-sw}}: ' +
            item.server +
            "</span>" +
            rr +
            '</div></div><button class="btn btn-warning btn-lg" onclick="showBackupInfo(' +
            "'" +
            item.name +
            "'" +
            ", " +
            ...
            item.description +
            "'" +
            ')"><span class="material-symbols-outlined">info</span></button><button class="btn btn-danger btn-lg" onclick="$.get(' +
            "'/backups/delete?filename=" +
            item.archive_name +
            "'" +
            '); refreshBackupsList();"><span class="material-symbols-outlined">delete</span></button><button class="btn btn-primary btn-lg" onclick="window.location=' +
            "'/backups/download?filename=" +
            item.archive_name +
            "'" +
            '"><span class="material-symbols-outlined">download</span></button><button class="btn btn-info btn-lg" onclick="restoreBackup(' +
            "'" +
            item.archive_name +
            "'" +
            ')"><span class="material-symbols-outlined">settings_backup_restore</span></button></li>'
        );
      });
    }
  });
