...
exports.getCoreURL = (core, cb) => {
  ...
  if (core_name == "Paper") {
    request.get("https://papermc.io/api/v2/projects/paper/versions/" + core_ver, options, (error, res, body) => {
      if (error) {
        console.log(error);
      };
      ...
      if (!error && res.statusCode == 200) {
        ...
        request.get("https://papermc.io/api/v2/projects/paper/versions/" + core_ver + "/builds/" + lastbuild, options, (error, res, body) => {
          if (error) {
            console.log(error);
          };
...
exports.getPaperCores = (cb) => {
  var jsona = [];
  request.get("https://papermc.io/api/v2/projects/paper", options, (error, res, body) => {
    if (error) {
...
exports.getSpigotCores = (cb) => {
  request.get("https://seeroy.github.io/spigots.json", options, (error, res, body) => {
    if (error) {
      return console.log(error)
    };
