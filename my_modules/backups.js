  ...
  if (fs.existsSync("./backups")) {
    ...
    read.forEach(function (fn) {
      if (path.extname(fn) == ".json") {
        ...
        rd['size'] = fs.lstatSync("./backups/" + fn.replace(".json", ".zip")).size;
        arr.push(rd);
      }
    });
...
exports.restoreBackup = function (fn, sn, cb) {
  if (fs.existsSync("./backups/" + fn)) {
    cfg = JSON.parse(fs.readFileSync("./backups/" + fn.replace(".zip", ".json")));
    if (cfg.type == "full") {
...
exports.createNewBackup = (bname, desc, type, sn, files = null) => {
  ...
  backups_processing[randuid].on('progress', function (pr) {
    backups_processing_status[randuid] = {
      ...
      percent: pr.entries.total > 0 ? Math.round(pr.entries.processed * 100.0 / pr.entries.total) : -1
    }
    fsock = io.sockets.sockets;
    for (const socket of fsock) {
      socket[1].emit("handleUpdate", {
  ...
  if (type == "full") {
    ...
    files.forEach(function (file) {
      if (file.type == "directory") {
        backups_processing[randuid].directory('./servers/' + sn + "/" + file.name, false);
      } else {
